...
(function ($) {
    ...
    jQuery(document).ready(function () {
        ...
        if(jQuery(".country-list.is-pro").length) {
            jQuery(".country-list.is-pro").select2({
                placeholder: "Select country",
                allowClear: false,
                closeOnSelect: false
            });
            $('.country-list.is-pro').on('select2:open', function (e) {
                if(jQuery(".select2-results__options").length) {
                    jQuery("body").removeClass("no-checkbox");
            ...
            jQuery(document).on("click", ".select-all-country", function(e){
                e.stopPropagation();
                jQuery(".country-list.is-pro option").each(function(){
                    jQuery(this).attr("selected", true);
                });
                jQuery('.country-list.is-pro').trigger("change");
                $('.country-list.is-pro').select2('close');

                var countryCount = parseInt($('.country-list.is-pro option:selected').length);
                if(countryCount == 0) {
                    $(".chaty-input-button").text("All countries");
                } else if(countryCount == 1) {
                    $(".chaty-input-button").text("1 country selected");
                } else {
                    $(".chaty-input-button").text(countryCount+" countries selected");
                }
            });
            ...
            jQuery(document).on("click", ".chaty-input-button", function(e){
                if(!jQuery(".country-list-box").hasClass("active")) {
                    jQuery(".country-list-box").addClass("active");
                    $('.country-list.is-pro').select2('open');
                } else {
                    jQuery(".country-list-box").removeClass("active");
                }
            });

            $('.country-list.is-pro').on('select2:close', function (e) {
                jQuery(".country-list-box").removeClass("active");
            });

            $('.country-list.is-pro').on('select2:select', function (e) {
                var countryCount = parseInt($('.country-list.is-pro option:selected').length);
                if(countryCount == 0) {
                    $(".chaty-input-button").text("All countries");
                } else if(countryCount == 1) {
            ...
            $('.country-list.is-pro').on('select2:unselect', function (e) {
                var countryCount = parseInt($('.country-list.is-pro option:selected').length);
                if(countryCount == 0) {
                    $(".chaty-input-button").text("All countries");
                } else if(countryCount == 1) {
        ...
        jQuery(document).on("click", ".color-field", function(){
...
(function ($) {
    jQuery(document).ready(function () {
        (function preview() {
            ...
            function previewPosition() {
                ...
                var customSpace = '10px';

                var value = jQuery('[name="cht_position"]:checked').val();

                if (value === 'right') {
                    $chatyWidget.css({right: customSpace, left: 'auto', bottom: '10px'});
                } else if (value === 'left') {
                    $chatyWidget.css({left: customSpace, right: 'auto', bottom: '10px'});
        ...
        socialIcon.on('click', function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    /*if(jQuery(".pre-set-message-whatsapp").length) {
                        jQuery(".pre-set-message-whatsapp").each(function() {
                            if(!jQuery(this).closest(".emojiPickerIconWrap").length) {
                                thisID = jQuery(this).prop("id");
                                jQuery("#" + thisID).emojiPicker({});
                            }
                        });
                    }*/
                },
        ...
        jQuery('#testUpload').on('change', function () {
            ...
            setTimeout(function(){
                detectIcon();
                if(jQuery(".js-upload").is(":checked")) {
                    jQuery(".js-upload").trigger("change");
                }
            },250);
        });
        ...
        function setIcon(icon, colorSelf) {
            ...
            if (colorSelf) {
                ...
                var color = jQuery("input[name='cht_color']:checked").val();
                if(color != "") {
                    var hashExists = color.indexOf("#");
                    if (hashExists == -1) {
                        color = "#" + color;
    ...
    jQuery(document).ready(function () {
        ...
        if(jQuery("#cht_color_custom").length) {
            jQuery("#cht_color_custom").spectrum({
                chooseText: "Submit",
                ...
                move: function (color) {
                    jQuery(this).val(color.toRgbString());
                    jQuery('#chaty-social-close ellipse').attr("fill", color.toRgbString());
                    jQuery('.preview .page #iconWidget svg circle, .chaty-close-icon svg ellipse').css({fill: color.toRgbString()});
                    jQuery("#cht_custom_color").val(color.toRgbString());
                    // jQuery('.color-picker-btn .circle').css({background: color.toRgbString()});
                },
                change: function (color) {
                    jQuery(this).val(color.toRgbString());
                    jQuery('#chaty-social-close ellipse').attr("fill", color.toRgbString());
        ...
        if(jQuery("#analytics_date").length) {
            jQuery("#analytics_date").datepicker();
        }

        if(jQuery(".select2-box").length) {
            jQuery(".select2-box").select2();

            $('.select2-box').on('select2:open', function (e) {
            ...
            if(confirm("Are you sure you want to delete this widget?")) {
                $.ajax({
                    ...
                    data: {
                        ...
                        widget_nonce: jQuery(this).data("nonce"),
                        widget_index: jQuery("#widget_index").val()
                    },
                    beforeSend: function (xhr) {
...
jQuery(document).ready(function () {
    ...
    jQuery("#create-data-and-time-rule").on("click", function () {
        ...
        jQuery('#gmt_'+totalDateAndTimeOptions+'_option').select2();
    ...
    if(totalDateAndTimeOptions > 0) {
        ...
        // for(var counter=0; counter<totalDateAndTimeOptions; counter++) {
        //     if(jQuery("#start_time_"+counter).length) {
        //         jQuery("#start_time_"+counter).timepicker({
        //             showLeadingZero: true
        //         });
        //     }
        //     if(jQuery("#end_time_"+counter).length) {
        //         jQuery("#end_time_"+counter).timepicker({
        //             showLeadingZero: true
        //         });
        //     }
        // }
    ...
    // if(jQuery("#cht_social_message_Whatsapp").length) {
    //     jQuery("#cht_social_message_Whatsapp").emojiPicker({
    //         // container: ".Whatsapp-settings .Whatsapp-settings"
    //     });
    // }
    ...
    /*jQuery(document).on("click", ".pre-message-whatsapp button", function(e){
        dataButton = jQuery(this).data("button");
        if(jQuery("#"+dataButton).length) {
            jQuery('#'+dataButton).emojiPicker('toggle');
        }
    });*/

    /*jQuery(document).on("click", ".emojiPickerIconWrap .emojiPickerIcon", function(e){
        //jQuery(".emojiPicker").toggle();
    ...
    jQuery('.form-fonts').on( 'change', function() {
        var font_val = jQuery('.form-fonts option:selected').text();
        jQuery('.chaty-google-font').remove();
        if (font_val != "" && jQuery(this).val() != "") {
            jQuery('.preview-section-chaty #admin-preview .page-body').css('font-family', font_val);
            if(jQuery('.form-fonts').length && jQuery('.form-fonts option:selected').data("group") != "Default") {
...
jQuery(window).on("load", function() {
    ...
    var font_val = jQuery('.form-fonts option:selected').text();
    jQuery('.chaty-google-font').remove();
    if (font_val != "" && jQuery('.form-fonts').val() != "") {
        if(jQuery('.form-fonts').length && jQuery('.form-fonts option:selected').data("group") != "Default") {
            jQuery('head').append('<link href="https://fonts.googleapis.com/css?family=' + font_val + ':400,600,700" rel="stylesheet" type="text/css" class="chaty-google-font">');
        }
        jQuery('.preview-section-chaty #admin-preview .page-body').css('font-family', font_val);
    ...
    jQuery("#chaty_icons_view").trigger("change");
});
...
function change_custom_preview() {
    ...
    if(jQuery("#channels-selected-list > li.chaty-channel").length <= 1) {
        jQuery(".form-horizontal__item.o-channel, .font-section").removeClass("active");
    } else {
        jQuery(".form-horizontal__item.o-channel, .font-section").addClass("active");
    }
}
