	...
	foreach ($optiondata as $key => $postdata) {
		...
		if ($FilterType == 'search_list'){
			...
			if( !empty($postdata['seapara']) ){
				foreach($postdata['seapara'] as $item => $val) {
					...
					if($TypeValue == 'taxonomy'){
						...
						}else if($FieldValue == 'alphabet' && !empty($DataValue)){
			...
			if( !empty($is_archivePage) ){
				if( $post_type == 'post' ){
					$args['cat'] = $ArchivepageID;
				}else{
					if( !empty($Archivepage) && !empty($ArchivepageType) && !empty($ArchivepageID) ){
						$attr_tax[] = array(
							array(
			...
			if($response['columns'] <= $PClimit){
				...
				for ($i=0; $i<$response['columns']; $i++){
					if($i < $PClimit){
						...
						$active = (($i+1) == 1) ? 'active' : '';
						$BtnNum .= '<button class="tp-pagelink tp-ajax-page hide '.esc_attr($active).'" data-page="'.esc_attr($i+1).'" >'.esc_html($i+1).'</button>';
					}
				}
			}
		}else{
	...
	{  
		...
		if(1 != $pages)
		{
			...
			if ( get_previous_posts_link() ){
				$paginate .= '<div class="paginate-prev">'.get_previous_posts_link('<i class="fas fa-long-arrow-alt-left" aria-hidden="true"></i>'.$pagination_prev).'</div>';
			}
			...
			if ( $paged < $pages ) $paginate .= "<a class='paginate-next' href='".get_pagenum_link($paged + 1)."'>".$pagination_next."<i class='fa fa-long-arrow-right' aria-hidden='true'></i></a>";
