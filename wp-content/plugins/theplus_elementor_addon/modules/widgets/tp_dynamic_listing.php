...
class ThePlus_Dynamic_Listing extends Widget_Base {
	...
	protected function render() {
		...
		if ( ! $query->have_posts() && $settings["blogs_post_listing"] != 'acf_repeater' ) {			
			...
			if(!empty($settings["blogs_post_listing"]) && ($settings["blogs_post_listing"] == 'search_list') ){
				if(is_archive()){
					global $wp_query;
					$query_var = $wp_query->query_vars;
					$post_category = $query_var['cat'];
			...
			if( (($settings['post_extra_option']=='load_more' || $settings['post_extra_option']=='lazy_load') && $layout!='carousel') || ( $FilterType == 'search_list' ) ){
				$postattr =[
					...
					'listing_type'=> !empty($settings['blogs_post_listing']) ? $settings['blogs_post_listing'] : 'page_listing',
				];
			...
			$output .= '<div id="pt-plus-dynamic-listing" class="dynamic-listing '.esc_attr($uid).' '.esc_attr($data_class).' '.esc_attr($style_layout).' '.$animated_class.' '.$child_filter_class.'" '.$layout_attr.' '.$data_attr.' '.$animation_attr.' data-enable-isotope="1">';
