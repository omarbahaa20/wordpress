...
class ThePlus_Search_Filter extends Widget_Base {
    ...
    protected function _register_controls() {
		...
		$this->add_responsive_control('TitleSvgIconTypo',
            [
                ...
                'selectors' => [
                    '{{WRAPPER}} .tp-search-filter .field-col .tp-field-title .tp-title-icon svg'=>'width:{{SIZE}}{{UNIT}}; height:{{SIZE}}{{UNIT}};',
                ],
            ]
        );
        ...
        $this->add_group_control(Group_Control_Typography::get_type(),
            [
                ...
                'label'=>esc_html__('Typography','theplus'),
                'scheme'=>Typography::TYPOGRAPHY_1,
                'selector'=>'{{WRAPPER}} .tp-search-filter .field-col .tp-column-result-wrap .tp-column-label',
            ]
        );
    ...
    protected function tp_checkbox( $data, $label, $name, $showCnt, $attr, $showchild, $Repeater ){
        ...
        $output .= $this->tp_filter_title($Repeater);
        $TPPrefix = $this->tp_unique_selector_id($name);
        $Taxonomy=!empty($Repeater['TaxonomyType']) ? $Repeater['TaxonomyType'] : '';
        $ImageON=!empty($Repeater['Imageshow']) ? $Repeater['Imageshow'] : '';
        $layout_style = !empty($Repeater['layout_style']) ? $Repeater['layout_style'] : 'style-1';
            ...
            $output .= '<div class="tp-checkBox '.esc_attr($layout_style).'"">';
                $output .= '<div class="tp-group">';
                    $output .= '<div class="tp-group-one">';
                        $output .= '<input type="checkbox" name="'.esc_attr($name).'" value="'.esc_attr($value).'" id="'.esc_attr($TPPrefix).esc_attr($value).'" data-title="'.esc_attr($DataName).'" />';
                    ...
                    $output .= '<div class="tp-child-taxo">';
                        foreach( $Datachild as $key => $child ){
                            ...
                            $output .= '<div class="tp-child-checkbox '.esc_attr($SubCName).'">';
                                $output .= '<input type="checkbox" name="'.esc_attr($name).'" value="'.esc_attr($TermId).'" id="'.esc_attr($TPPrefix).esc_attr($TermId).'" data-title="'.esc_attr($ChildName).'" />';
        ...
        $output .= '<div class="tp-wp-radio tb-checkBox-data" data-tpprefix="'.esc_attr($TPPrefix).'">';
            foreach($data as $value => $label){
                ...
                $output .= '<div class="tp-radio '.esc_attr($layout_style).'">';
                    $output .= '<div class="tp-group">';
                        $output .= '<div class="tp-group-one">';
                            $output .= '<input type="radio" name="'.esc_attr($name).'" value="'.esc_attr($value).'" id="'.esc_attr($TPPrefix).esc_attr($value).'" data-title="'.esc_attr($RadioName).'" />';
                            $output .= '<label for="'.esc_attr($TPPrefix).esc_attr($value).'" class="tp-lable">';
    ...
    protected function tp_date($title,$key,$Repeater){
        ...
        if($layout_date == 'style-1'){
            $op .= '<div class="tp-date-wrap '.esc_attr($DateStyle).' '.esc_attr($layout_date).'">';
            ...
            $op .= '<div class="tp-date-wrap '.esc_attr($layout_date).'" data-CustomDate= \''.$CustomDate.'\'>';
                $op .= '<input type="text" name="tp-date" id="'.esc_attr($key).'" class="tp-custom-date '.esc_attr($key).'" date-key="'.esc_attr($key).'" />';
            $op .= '</div>';
        }
    ...
    protected function tp_unique_selector_id($name){
        $TPPrefix = '';

        $GetWDID = explode( "-", $this->get_unique_selector() );
        if( is_array($GetWDID) ){
            $GetWDKey = array_key_last($GetWDID);
            $TPPrefix = 'tp-'.esc_attr($name).'-'.esc_attr($GetWDID[$GetWDKey]);
        }
