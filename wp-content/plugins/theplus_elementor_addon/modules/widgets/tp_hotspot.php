				...
				if(!empty($settings['pin_hotspot'])) {
					...
					foreach($settings['pin_hotspot'] as $item) {
						...
						if($item['select_option'] == 'icon'){
							...
							if(!empty($item["pin_image"]["url"])){
								...
								$image= tp_get_image_rander( $image_id,$item['pin_thumbnail_size'], [ 'class' => 'pin-icon' ] );
							}							
					...
					if((!empty($item['hs_link_switch']) && $item['hs_link_switch'] == 'yes') && !empty($item['hs_link']['url'])){
						$pin_loop .= '<a '.$this->get_render_attribute_string( $link_key ).'>';
							...
							$pin_loop .= '<div id="'.esc_attr($uid_loop).'" class="pin-hotspot-loop '.esc_attr($uid_loop).' elementor-repeater-item-'.esc_attr($item['_id']). ' '.esc_attr($animated_strager_class).'" '.$this->get_render_attribute_string( '_tooltip' ).'>';
