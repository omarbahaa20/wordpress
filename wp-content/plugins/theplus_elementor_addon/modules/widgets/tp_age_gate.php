...
class ThePlus_Age_Gate extends Widget_Base {
	...
	protected function render() {
		...
		$button_icon=$second_button_icon=$right_img_class=$bg_image_main_wrapper=$data_attr='';		
		...
		if((\Elementor\Plugin::$instance->editor->is_edit_mode()) && $settings["backend_preview"] != 'yes'){
			...
			$output ='<div class="tp-agegate-wrapper tp-'.$settings['age_verify_method'].'" '.$bg_image_main_wrapper.' '.$data_attr.'>';
				$output .='<div class="tp-agegate-inner-wrapper">';
						...
						if(!empty($age_side_image_show) && $age_side_image_show=='yes' && !empty($settings['age_side_img']['url'])){
							$output .='<div class="tp-agegate-boxes '.$right_img_class.'" style="background-image:url('.$settings['age_side_img']['url'].');background-size:cover;   background-attachment:inherit;">';
							$output .='</div>';
						}				
				$output .='</div>';
			$output .='</div>';
