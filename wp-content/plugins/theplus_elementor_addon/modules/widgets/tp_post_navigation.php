		...
		if (!empty( $prev_post )){
			...
			if (has_post_thumbnail( $prev_post->ID ) ){
				$prevpostimg .= '<div class="post-image">';
					$prevpostimg .= get_the_post_thumbnail( $prev_post->ID, 'thumbnail', array( 'class' => 'tp-nav-trans' ) );
				$prevpostimg .= '</div>';
			}else{
				$prevpostimg .= '<div class="post-image">';
			...
			if(!empty($style)){
				if($style=='style-1'){	
					...
					$prevnav .= '<a href="'.esc_url(get_permalink( $prev_post->ID )).'" class="post_nav_link prev tp-nav-trans" rel="'.esc_attr__('prev','theplus').'">';
					$prevnav .='<div class="tp-post-nav-hover-con" style="background-image: url('.esc_url(!empty($img[0]) ? $img[0] : '').');background-size: cover;background-attachment: fixed;background-position: center center;background-repeat:no-repeat;">'.$prevpostcon.'</div></a>';
		...
		if (!empty( $next_post )){
			...
			if (has_post_thumbnail( $next_post->ID ) ){
				$nextpostimg .= '<div class="post-image">';
					$nextpostimg .= get_the_post_thumbnail( $next_post->ID, 'thumbnail', array( 'class' => 'tp-nav-trans' ) );
				$nextpostimg .= '</div>';
			}else{
				$nextpostimg .= '<div class="post-image">';
			...
			if(!empty($style)){
				if($style=='style-1'){					
					...
					$nextnav .= '<a href="'.esc_url(get_permalink( $next_post->ID )).'" class="post_nav_link next tp-nav-trans" rel="'.esc_attr__('next','theplus').'">';
					$nextnav .='<div class="tp-post-nav-hover-con" style="background-image: url('.esc_url($img1[0]).');background-size: cover;background-attachment: fixed;background-position: center center;background-repeat:no-repeat;">'.$nextpostcon.'</div></a>';
				}else if($style=='style-4'){				
					$nextnav .= '<a href="'.esc_url(get_permalink( $next_post->ID )).'" class="post_nav_link next tp-nav-trans" rel="'.esc_attr__('next','theplus').'">';
						$nextnav .=$nextpostimg;
