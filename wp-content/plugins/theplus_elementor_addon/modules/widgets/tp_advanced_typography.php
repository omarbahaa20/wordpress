					...
					if(!empty($advanced_typography_text)){
						...
						if($settings['marquee_switch']=='yes'){
							if(!empty($settings['marquee_type']) && $settings['marquee_type']=='on_transition'){
								$adv_typ_text = '<div id="'.esc_attr($uidat).'" class="tp_adv_typo_'.esc_attr($settings['marquee_direction']).' text-content-block  tp-adv-typ-marquee '.esc_attr($circular_text_switch).' '.esc_attr($stroke_switch).' '.esc_attr($background_based_text_switch).' '.esc_attr($img_gif_ovly_txtimg_switch).' '.esc_attr($on_hover_img_reveal_switch).' '.esc_attr($text_continuous_animation).' '.esc_attr($animated_class).'" '.$data_attr.' style="white-space: nowrap;'.$text_write_mode.$mix_blend_attr.'" '.$circular_attr.' '.$animation_attr.'>'.htmlspecialchars_decode($loop_text).'</div>';
							}else{
								$marquee_attr='';
							...
							if (!empty($settings['text_link']['url'])){								
								...
								$adv_typ_text .= '<div id="'.esc_attr($uidat).'" class="text-content-block  '.esc_attr($circular_text_switch).' '.esc_attr($stroke_switch).' '.esc_attr($background_based_text_switch).' '.esc_attr($img_gif_ovly_txtimg_switch).' '.esc_attr($on_hover_img_reveal_switch).' '.esc_attr($text_continuous_animation).' '.esc_attr($animated_class).'" '.$data_attr.' style="'.$text_write_mode.$mix_blend_attr.'" '.$circular_attr.' '.$animation_attr.'>'.htmlspecialchars_decode($loop_text).'</div>';
