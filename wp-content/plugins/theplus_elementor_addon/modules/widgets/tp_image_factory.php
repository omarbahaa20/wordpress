		...
		$animated_direction = $settings['animated_direction'];
			...
			if ( ! empty( $settings['link']['url'] ) ) {
				if($animated_style == 'animate-image'){	
					$html = '<a ' . $this->get_render_attribute_string( 'link' ) . ' class="' . esc_attr($wrapperClass) . ' pt-plus-bg-image-animated '.esc_attr($animated_direction).'" style="'.esc_attr($data_image).'" >' .$content_image. '</a>';
				}else{
