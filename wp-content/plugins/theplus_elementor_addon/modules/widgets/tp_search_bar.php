...
class ThePlus_Search_Bar extends Widget_Base {
    ...
    protected function _register_controls() {
		...
		$this->add_control('SpecialCTP',
			[
				'label'=>__('Only for specific CTP','theplus'),
				'type'=>Controls_Manager::SWITCHER,
				'label_on'=>__('Show','theplus'),
				'label_off'=>__('Hide','theplus'),
				'return_value'=>'yes',
		...
		$this->add_control('SpecialCTPType',
			[
				'label'=>esc_html__('SpecialCTP Type','theplus'),
				'type'=>Controls_Manager::SELECT,
				'default'=>'post',
                'options'=>theplus_get_post_type(),
                'condition'=>[
		...
		$this->add_control('imagewidth',
			[
				...
				'selectors'=>[
					'{{WRAPPER}} .tp-search-bar .tp-search-area .tp-search-list .tp-item-image'=>'width:{{SIZE}}{{UNIT}};',
				],
				'condition'=>[
					'ajaxsearch'=>'yes',
					'ThubOn'=>'yes',
		...
		$this->add_control('errorpadding',
			[
				...
				'size_units'=>['px','%'],
				'selectors'=>[
					'{{WRAPPER}} .tp-search-area .tp-search-error'=>'padding:{{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
	...
	protected function render() {
		...
		if(!empty($temp)){
			foreach($temp as $idx => $item){
				...
				if($STY == 'post'){
					foreach($item['postType'] as $Id1 => $item1){
						$Defa_Postype[] = $item1;
					}
				}
			}
		...
		$dataattr=[];
		$dataattr['ajax'] = !empty($settings['ajaxsearch']) ? 'yes' : 'no';
		$dataattr['ajaxsearchCharLimit'] = !empty($settings['ajaxsearchCharLimit']) ? $settings['ajaxsearchCharLimit'] - 1 : 2;
		
		$dataattr['nonce'] = wp_create_nonce("tp-searchbar");
		$dataattr['style'] = $RStyle;
		$dataattr['styleColumn'] = $Rcolumn;
		$dataattr['post_page'] = (!empty($settings['postCount']) && !empty($settings['postCount']['size'])) ? (int)$settings['postCount']['size'] : 3;
		$dataattr['Postype_Def'] = $Defa_Postype;

		$dataattr = htmlspecialchars(json_encode($dataattr), ENT_QUOTES, 'UTF-8');

		$output .= '<div class="tp-search-bar '.esc_attr($Backclass).' '.esc_attr($WidgetId).'" data-id="'.esc_attr($WidgetId).'" data-ajax_search= \''.$dataattr.'\' data-genericfilter='.$GFarray.' data-result-setting= \''.$ResultOnOff.'\' data-pagination-data= \''.$PageJson.'\' data-acfdata='.esc_attr($AcfData).' data-default-data= \''.$DefaultSetting.'\' >';
			if($Overlay=='yes'){
				$output .= '<div class="tp-rental-overlay"></div>';
			}	

			$output .= '<form class="tp-search-form" method="get" action="'.esc_url(site_url()).'">';
			...
			$output .= '<div class="tp-search-area '.esc_attr($RStyle).'">';
				...
				$output .= '<div class="tp-search-list">';
					$output .= '<div class="tp-search-list-inner '.esc_attr($scrollclass).'"></div>';
				$output .= '</div>';
		...
		$output .= '<div class="tp-input-field">';
			$output .= '<div class="tp-input-label-field">';
				if(!empty($searchLabel)){
					$output .= '<label class="tp-search-label">'.esc_html__($searchLabel,'theplus').'</label>';
				}
			$output .= '</div>';
	...
	protected function tp_search_drop_down($data, $name, $id, $taxo, $repeater){
		...
		if( !empty($label) ){
			$output .= '<label class="tp-search-label">'.esc_html__($label,'theplus').'</label>';
		}
		...
		$output .= '<div class="tp-sbar-dropdown">';
			$output .= '<div class="tp-select">';
				$output .= '<span>'.esc_html__($DefText).'</span><i class="fas fa-chevron-down tp-dd-icon"></i>';
			$output .= '</div>';
			$output .= '<input type="hidden" name="'.$DatName.'" id="'.$DatName.'" >';

			$output .= '<ul class="tp-sbar-dropdown-menu">';

				$output .= '<li id="" class="tp-searchbar-li">';
					$output .= esc_html__($DefText);
				$output .= '</li>';
				...
				foreach($data as $key => $label){
					...
					$output .= '<li id="'.esc_attr($key).'" class="tp-searchbar-li" >';
						if( !empty($showCnt) ){
							$output .= esc_html__($LName) .' ('.esc_html__($Lcount).')';
						}else{
							$output .= esc_html__($LName);
						}
					$output .= '</li>';
				}
			$output .= '</ul>';
		...
		if(!empty($showBtn)){
			...
			$output .= '<div class="tp-btn-wrap">';
				$output .= '<button class="tp-search-btn" name="submit" >';
					...
					if(!empty($BtnText)){
						$output .= '<span class="tp-search-btn-txt '.esc_attr($BtnPos).'">'.esc_html__($BtnText).'</span>';
					}
