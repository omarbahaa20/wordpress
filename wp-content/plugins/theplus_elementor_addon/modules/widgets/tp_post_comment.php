...
class ThePlus_Post_Comment extends Widget_Base {
			...
			echo '<div class="tp-post-comment tp-widget-'.esc_attr($uid_pscmnt).'">';
				echo '<div id="comments" class="comments-area">';
					...
					echo '<ul class="comment-list">';
						echo '<li>';
							echo '<div class="comment-section-title">'.esc_html__('Comment', 'theplus').' ('. get_comments_number($post_id) . ')</div>';
						echo '<li>'; 
	...
	public function tp_comment_args(){
		...
		$args = array(
		  ...
		  'id_submit'         => 'submit',
		  'title_reply'       => esc_html__( 'Leave Your Comment', 'theplus' ),
		  'title_reply_to'    => esc_html__( 'Leave a Reply to %s', 'theplus' ),
		  'cancel_reply_link' => esc_html__( 'Cancel Reply', 'theplus' ),
		  'label_submit'      => esc_html__( 'Submit Now', 'theplus' ),

		  'comment_field' =>  '<div class="tp-row"><div class="tp-col-md-12 tp-col"><label><textarea id="comment" name="comment" cols="45" rows="6" placeholder="'.esc_attr__('Comment','theplus').'" aria-required="true"></textarea></label></div></div>',
