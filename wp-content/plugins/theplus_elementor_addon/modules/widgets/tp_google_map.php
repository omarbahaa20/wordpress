	 ...
	 protected function render() {
		...
		$hue = (!empty($settings["hue"]) ? $settings["hue"] : '#ccc');
			...
			$gmap_content ='<div class="pt-plus-adv-gmap">';
				$gmap_content .='<div id="'.esc_attr($uid).'" class="pt-plus-adv-map js-el '.esc_attr($animated_class).'" data-id="'.esc_attr($uid).'" data-adv-maps="'.htmlentities($json, ENT_QUOTES, "UTF-8").'" data-map-style="'.esc_attr($maps_style).'"  '.$animation_attr.'></div>';
				...
				if(!empty($settings["overlay_toggle"]) && $settings["overlay_toggle"]=='yes'){
					...
					$gmap_content .='<div class="pt-plus-overlay-map-content '.esc_attr($uid).' '.esc_attr($lz1).'"  data-uid="'.esc_attr($uid).'" data-toggle-btn-color="'.esc_attr($toggle_btn_color).'" data-toggle-active-color="'.esc_attr($toggle_ative_color).'">';
