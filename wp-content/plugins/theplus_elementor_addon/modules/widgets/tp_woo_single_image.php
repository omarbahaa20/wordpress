					...
					if(!empty($select) && $select=='single_product'){
						...
						//if ($attachment_ids) {
							...
							if(!empty($hover_image_on_off) && $hover_image_on_off=='yes' && !empty($attachment_ids)){
								echo '<span class="product-image hover-image">'.wp_get_attachment_image( $attachment_ids[0], $sp_thumbnail ).'</span>';
							}						
							echo wp_get_attachment_image( get_post_thumbnail_id(), $sp_thumbnail );
							if(isset($settings['on_image_link']) && $settings['on_image_link']=='yes'){
								echo '</a>';
							}							
						//}
							...
							$output .= '<div id="tp-woo-gallery" class="tp-woo-gallery '.esc_attr($uid).' '.esc_attr($data_class).' " '.$layout_attr.' '.$data_attr.'  data-enable-isotope="1">';
								...
								foreach( $attachment_ids as $attachment_id ){
									...
									if(!empty($layout) && $layout=='metro'){
										...
										$featured_image = '<div class="woo-gallery-bg-image-metro" '.$featured_image.'></div>';
									}
									
									if(!empty($layout) && $layout=='grid'){	
										$featured_image=wp_get_attachment_image_src($attachment_id,'tp-image-grid');									
										$featured_image='<img src="'.esc_url($featured_image[0]).'">';
									...
									$attachment_ids = $product->get_gallery_image_ids();
									if(!empty($layout) && $layout=='grid'){	
										echo wp_get_attachment_image( get_post_thumbnail_id(), 'tp-image-grid' );
									}else if(!empty($layout) && $layout=='masonry'){		
