...
class ThePlus_Woo_Thank_You extends Widget_Base {
	...
	public function render() {
		...
		if(class_exists('woocommerce')){
			$tp_thankyou_or_txt = $settings['tp_thankyou_or_txt'];		
			$tp_thankyou_order_detail_head = $settings['tp_thankyou_order_detail_head'];
			...
			if(!empty($settings['tp_thankyou_order_main']) && $settings['tp_thankyou_order_main']=='yes'){
				if ( $order ){
						if ( $order->has_status( 'failed' ) ) { 						
							?>
							<p class="woocommerce-notice woocommerce-notice--error woocommerce-thankyou-order-failed"><?php esc_html_e( 'Unfortunately your order cannot be processed as the originating bank/merchant has declined your transaction. Please attempt your purchase again.', 'theplus' ); ?></p>

							<p class="woocommerce-notice woocommerce-notice--error woocommerce-thankyou-order-failed-actions">
								<a href="<?php echo esc_url( $order->get_checkout_payment_url() ); ?>" class="button pay"><?php esc_html_e( 'Pay', 'theplus' ); ?></a>
								<?php if ( is_user_logged_in() ) : ?>
									<a href="<?php echo esc_url( wc_get_page_permalink( 'myaccount' ) ); ?>" class="button pay"><?php esc_html_e( 'My account', 'theplus' ); ?></a>
								<?php endif; ?>
							</p>
							<?php
						}else{ 
								...
								echo '<ul class="woocommerce-order-overview woocommerce-thankyou-order-details order_details '.$tp_t_oo_layout.'">';
										?>
												<li class="woocommerce-order-overview__order order">
													<?php esc_html_e( 'Order number:', 'theplus' ); ?>
													<strong><?php echo $order->get_order_number(); ?></strong>
												</li>
									
												<li class="woocommerce-order-overview__date date">
													<?php esc_html_e( 'Date:', 'theplus' ); ?>
									...
									if ( $order->get_payment_method_title() ) {
												...
												<li class="woocommerce-order-overview__payment-method method">
													<?php esc_html_e( 'Payment method:', 'theplus' ); ?>
			...
			if(!empty($settings['tp_thankyou_order_detail']) && $settings['tp_thankyou_order_detail']=='yes'){
				...
				<section class="woocommerce-order-details">
					...
					if(!empty($tp_thankyou_order_detail_head)){ ?>
						<h2 class="woocommerce-order-details__title"><?php esc_html_e( $tp_thankyou_order_detail_head, 'theplus' ); ?></h2>
						<?php
					}
					?>
			...
			if(!empty($settings['tp_thankyou_billing']) && $settings['tp_thankyou_billing']=='yes'){
				...
				<h2 class="woocommerce-column__title"><?php esc_html_e( 'Billing address', 'theplus' ); ?></h2>

				<address>
					<?php echo wp_kses_post( $order->get_formatted_billing_address( esc_html__( 'N/A', 'theplus' ) ) ); ?>
			...
			if(!empty($settings['tp_thankyou_shipping']) && $settings['tp_thankyou_shipping']=='yes'){ ?>
				...
				<h2 class="woocommerce-column__title"><?php esc_html_e( 'Shipping address', 'theplus' ); ?></h2>
				<address>
					<?php echo wp_kses_post( $order->get_formatted_shipping_address( esc_html__( 'N/A', 'theplus' ) ) ); ?>
				</address>
			</div>
			<?php
