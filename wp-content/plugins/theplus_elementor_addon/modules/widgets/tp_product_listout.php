...
class ThePlus_Product_ListOut extends Widget_Base {
    ...
    protected function _register_controls() {
		...
		$this->add_control('search_list_note',
			[
				'label'=>esc_html__( 'Note : This feature not works in Carousel Layout.','theplus'),
		...
		$this->add_control(
			'display_rating',
			[
				'label' => esc_html__( 'Display Rating', 'theplus' ),
				'type' => Controls_Manager::SWITCHER,
				'label_on' => esc_html__( 'Hide', 'theplus' ),
				'label_off' => esc_html__( 'Show', 'theplus' ),
				'return_value' => 'yes',
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Compare', 'theplus' ),
				'type' => Controls_Manager::SWITCHER,
				'label_on' => esc_html__( 'Show', 'theplus' ),
				'label_off' => esc_html__( 'Hide', 'theplus' ),
				'default' => 'no',
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Wishlist', 'theplus' ),
				'type' => Controls_Manager::SWITCHER,
				'label_on' => esc_html__( 'Show', 'theplus' ),
				'label_off' => esc_html__( 'Hide', 'theplus' ),
				'default' => 'no',
	 ...
	 protected function render() {
		...
		if ( !class_exists('woocommerce') ) {
			...
			if(!empty($settings["product_post_listing"]) && ($settings["product_post_listing"]=='search_list') ){
				if(is_archive()){
					global $wp_query;
					$ProductName='';
					$query_var = $wp_query->query_vars;
					...
					$ArchivePage = array(
						...
						'archive_Id' => ($GetId) ? $GetId->term_id : 0,
					);
				}
			...
			if( (($settings['post_extra_option']=='load_more' || $settings['post_extra_option']=='lazy_load') && $layout!='carousel') || ($FilterType == 'search_list') ){
				$postattr =[
					'load' => 'products',
					'post_type' => 'product',
					...
					'listing_type'=> !empty($settings['product_post_listing']) ? $settings['product_post_listing'] : 'page_listing',
				];				
			...
			$output .= '<div id="'.esc_attr($uid).'" class="tp-row post-inner-loop '.esc_attr($uid).' '.esc_attr($content_alignment).' tp_list" '.$serchAttr.' '.$TotProduct.' >';
