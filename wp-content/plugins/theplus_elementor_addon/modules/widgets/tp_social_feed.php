...
class ThePlus_Social_Feed extends Widget_Base {
    ...
    protected function _register_controls() {
		...
		$this->start_controls_section(
			'social_feed_section',
			[
				'label' => esc_html__( 'Social Feed Option', 'theplus' ),
				'tab' => Controls_Manager::TAB_CONTENT,
			]
		);
		$this->add_control(
		...
		$this->add_control(
			...
			[
				...
				'separator' => 'before',
				'condition'   => [
					'FancyStyle' => ['default','style-1','style-2'],
				],
			]
		);
		$this->add_group_control(
	...
	protected function render() {
        ...
        $output .= '<div id="'.esc_attr($uid_sfeed).'" class="'.esc_attr($uid_sfeed).' tp-social-feed '.esc_attr($data_class).'" '.$layout_attr.' '.$data_attr.' data-fancy-option=\''.$fancybox_settings.'\' data-scroll-normal='.esc_attr($NormalScroll).' data-enable-isotope="1">';
			...
			$FinalData = [];
			$Perfo_transient = get_transient("SF-Performance-$WidgetUID");
			if( ($Performance == false) || ($Performance == true && $Perfo_transient === false) ){
				...
				set_transient("SF-Performance-$WidgetUID", $FinalData, $RefreshTime);
			}else{
				$FinalData = get_transient("SF-Performance-$WidgetUID");
			}
		    ...
		    if(!empty($FinalData)){
				...
				if($postLodop == 'load_more' || $postLodop == 'lazy_load'){
					...
					if(get_transient("SF-Loadmore-$uid_sfeed") === false){
						set_transient("SF-Loadmore-$uid_sfeed", $remindata , $RefreshTime);
				    }

					$postattr = [
						'load_class' => esc_attr($uid_sfeed),
	...
	protected function InstagramFeed($social){
		...
		if($IGType == "Instagram_Basic"){
			...
			if($UserID_CODE == 200){
				$GET_UserID = !empty($UserID_Res['instagram_business_account']) ? $UserID_Res['instagram_business_account']['id'] : 'jack.jonas.124';
				...
				if($IGFeedType == 'IGUserdata'){
					...
					$UserPost_API = "{$BashURL}/{$GET_UserID}?fields=business_discovery.username({$IGUserName}){username,profile_picture_url,followers_count,media_count,media.limit({$IGGPcount}){permalink,media_type,media_url,like_count,comments_count,timestamp,video_title,caption,id,media_product_type,children{media_url,permalink,media_type}}}&access_token={$IGAcT}";
