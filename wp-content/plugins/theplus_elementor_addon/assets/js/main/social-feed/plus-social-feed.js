...
(function($) {
    ...
    var WidgetSocialFeedHandler = function($scope, $) {
        var container = $scope.find('.tp-social-feed');
            ...
            let e = $(this),
                ...
                $().fancybox({
                    ...
                    afterShow: function() {
                        var $self = this,
                            FeedType = $self.$content[0].dataset.fancyfeedtype;
                            if(FeedType == "Instagram"){
                                var getID = $self.src;
                                var Getslider = $(getID).find('#IGGP-slider');
                                    if(Getslider.length){
                                        Getslider.slick({
                                            speed: 1000,
            ...
            $('.grid-item.feed-Facebook.FbAlbum_Cls', this).each(function() {
            ...
            if (Get_SN.ScrollOn == true && Get_SN.TextLimit == false) {
                let SF_Text = e.find('.tp-sf-feed .tp-message');
            ...
            $(document).on( 'click' , "."+BoxID+".tp-social-feed .readbtn", function() {
                ...
                if (div.hasClass('show-text')) {
                    ...
                    div.addClass('show-text ');
                    $(this).html('Show Less');
                    div.find('.sf-dots').css('display', 'none');

                    let SF_Text = $('.tp-social-feed').find(S);
                    if (Scroll.ScrollOn == true && Scroll.TextLimit == true) {
                        SF_Text.each(function() {
            ...
            $(document).on( 'click' , ".tp-widget-"+BoxID+".fancybox-si .readbtn", function() {
                var div = $(this).closest('.fancybox-si .tp-message'),
                    Scroll = $('.tp-social-feed').data("scroll-normal"),
                    FcyMsg = $(this).closest('.fancybox-si .tp-message');
            ...
            // LoadMore
            var i = 0;
            if ($("."+BoxID+".tp-social-feed").find('.ajax_load_more').length > 0) {
                $("."+BoxID+" .post-load-more").on('click', function(e) {
                    ...
                    $.ajax({
                            type: 'POST',
                            data: 'action=tp_feed_load&view=' + i + '&feedshow=' + loadFview + '&loadattr=' + loadFeed,
                            ...
                            complete: function() {
                                ...
                                if (loadlayout == 'grid' || loadlayout == 'masonry') {
                                    if (!$("." + loadFeed.load_class).hasClass("list-grid-client")) {
                                        setTimeout(function() {
                                            $("." + loadclass + ' .post-inner-loop').isotope('layout').isotope('reloadItems');
                                        }, 500);
                                    }
                                }
                            }
